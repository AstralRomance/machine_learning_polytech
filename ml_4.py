...
def svm_point2():
    ...
    for i in range(1, 500):
        ...
        if metrics.accuracy_score(res_train, clf.predict(data_train)) >= 1.0:
            print('current C: {0}; train dataset'. format(i))
        if metrics.accuracy_score(res_test, clf.predict(data_test)) >= 1.0:
            print('current C: {0}; test dataset'.format(i))
...
def svm_point3():
    ...
    for i in ['poly', 'rbf', 'sigmoid']:
        if i == 'poly':
            ...
            clf = SVC(kernel=i, C=1.0, gamma='auto')
            clf.fit(data_train, res_train)
            print('current accuracy: {0}; current kernel: {1}'.format(metrics.accuracy_score(res_train,
                                                                                                            clf.predict(data_train)), i))
...
def svm_point4():
    ...
    for i in ['poly', 'rbf', 'sigmoid']:
        if i == 'poly':
            ...
            clf = SVC(kernel=i, C=1.0, gamma='auto')
            clf.fit(data_train, res_train)
            print('current accuracy: {0}; current kernel: {1}'.format(metrics.accuracy_score(res_test,
                                                                                                            clf.predict(data_test)), i))
